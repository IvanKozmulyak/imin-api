...
**Key mode is recorded, not assumed.** `StripeProperties.isLiveKey()` trims the key and matches `^(sk|rk)_live_`; `StripeConfig` logs the detected mode at startup (`STRIPE MODE: live|test`, at WARN — an ERROR there would file a Sentry issue on every boot). `organizations.stripe_livemode` (V129, nullable) is stamped at account creation from that answer, and both `getStatus` and `getStatusLive` refuse an org whose recorded mode contradicts the running key — logged as an ERROR, reported as not-connected, so checkout answers its existing leak-safe 404 instead of selling into an account that cannot receive a cent, and the dashboard shows the connect CTA. **`NULL` means unknown and is always allowed**, so no un-stamped org can be bricked. A mismatched org recovers by itself: `getOrCreateAccount` treats the other mode's account as absent, mints a replacement under the running key and resets the Connect mirror. Related: `test_mode` on `orders`, `payout_runs` and `disputes` (V130) marks test-era money; those rows are excluded from every input to the per-event payout net (gross, refunds, application fee, disputed face value, already-triggered amount), from `findPayoutCandidates`/`findRetentionMonitorCandidates`, and from the parked-run guard `existsBlockedNeedingAHuman` (a test-era park must not withhold a mixed event's live sales forever) — and from nothing else. The one-off test→live reset and its runbook are `scripts/stripe-live-cutover*.sql` + `docs/STRIPE_LIVE_CUTOVER.md`.

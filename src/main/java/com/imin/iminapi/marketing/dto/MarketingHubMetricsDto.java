 ...
 *   <li>{@code attributedRevMinor} — attributed revenue in minor units. No per-campaign
 *       revenue linkage exists in the model today (orders carry no utm key — see
 *       {@code AttributionService}), so this is 0 until that machinery ships.</li>
 *   <li>{@code attributedPurchases} — attributed checkout sessions summed across the org's
 *       campaigns from the last 30 days (utm_campaign funnel read-model).</li>
